...
struct piranha_audio_device {
    ...
    struct piranha_stream_out *outputs[OUTPUT_TOTAL];
    bool mic_mute;
    int tty_mode;
...
struct piranha_stream_out {
    ...
    struct piranha_audio_device *dev;
};
...
static void select_output_device(struct piranha_audio_device *adev);
static void select_input_device(struct piranha_audio_device *adev);
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume);
static int do_input_standby(struct piranha_stream_in *in);
static int do_output_standby(struct piranha_stream_out *out);
static void in_update_aux_channels(struct piranha_stream_in *in, effect_handle_t effect);
...
{
    struct piranha_audio_device *adev = (struct piranha_audio_device *)data;
...
static void set_incall_device(struct piranha_audio_device *adev)
{
    int device_type;
    ...
    switch(adev->out_device) {
        ...
        case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
            ...
            break;
/*
 * piranha doesn't need a special path set for bluetooth
 * 
...
static void set_input_volumes(struct piranha_audio_device *adev, int main_mic_on,
...
static void set_output_volumes(struct piranha_audio_device *adev, bool tty_volume)
{
    unsigned int channel;
    int speaker_volume;
    int headset_volume;
...
static void force_all_standby(struct piranha_audio_device *adev)
{
    struct piranha_stream_in *in;
...
static void select_mode(struct piranha_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
...
static int start_output_stream_low_latency(struct piranha_stream_out *out)
{
    struct piranha_audio_device *adev = out->dev;
#ifdef PLAYBACK_MMAP
    ...
    if (adev->out_device & ~(AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET | AUDIO_DEVICE_OUT_AUX_DIGITAL)) {
        ...
        out->pcm[PCM_NORMAL] = pcm_open(CARD_piranha_DEFAULT, PORT_TONES,
    ...
    if (adev->out_device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET) {
        ...
        out->pcm[PCM_SPDIF] = pcm_open(CARD_piranha_DEFAULT, PORT_SPDIF,
...
/* must be called with hw device and output stream mutexes locked */
static int start_output_stream_deep_buffer(struct piranha_stream_out *out)
{
    struct piranha_audio_device *adev = out->dev;
    ...
    out->config[PCM_NORMAL].rate = MM_FULL_POWER_SAMPLING_RATE;
    out->pcm[PCM_NORMAL] = pcm_open(CARD_piranha_DEFAULT, PORT_MM,
...
static int start_output_stream_hdmi(struct piranha_stream_out *out)
{
    struct piranha_audio_device *adev = out->dev;
    ...
    if (adev->outputs[OUTPUT_LOW_LATENCY] != NULL &&
            !adev->outputs[OUTPUT_LOW_LATENCY]->standby) {
        struct piranha_stream_out *ll_out = adev->outputs[OUTPUT_LOW_LATENCY];
...
static void add_echo_reference(struct piranha_stream_out *out,
...
static void remove_echo_reference(struct piranha_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct piranha_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct piranha_audio_device *adev,
...
static int get_playback_delay(struct piranha_stream_out *out,
                       size_t frames,
...
{
    struct piranha_stream_out *out = (struct piranha_stream_out *)stream;
...
static int start_input_stream(struct piranha_stream_in *in)
{
    int ret = 0;
    struct piranha_audio_device *adev = in->dev;
...
static void get_capture_delay(struct piranha_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct piranha_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct piranha_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct piranha_stream_in *)((char *)buffer_provider -
                                   offsetof(struct piranha_stream_in, buf_provider));
...
static ssize_t read_frames(struct piranha_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct piranha_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static int in_configure_reverse(struct piranha_stream_in *in)
{
    int32_t cmd_status;
    uint32_t size = sizeof(int);
...
static void in_read_audio_effect_channel_configs(struct piranha_stream_in *in,
...
static uint32_t in_get_aux_channels(struct piranha_stream_in *in)
{
    int i;
...
static int in_reconfigure_channels(struct piranha_stream_in *in,
...
static void in_update_aux_channels(struct piranha_stream_in *in,
                                   effect_handle_t effect)
{
    uint32_t aux_channels;
    channel_config_t channel_config;
...
static int out_read_hdmi_channel_masks(struct piranha_stream_out *out) {
    int max_channels = 0;
    ...
    out->sup_channel_masks[0] = AUDIO_CHANNEL_OUT_5POINpiranha;
    if (max_channels == 8)
        out->sup_channel_masks[1] = AUDIO_CHANNEL_OUT_7POINpiranha;
...
{
    struct piranha_audio_device *ladev = (struct piranha_audio_device *)dev;
    struct piranha_stream_out *out;
    int ret;
    int output_type;
    ...
    *stream_out = NULL;
    ...
    if (flags & AUDIO_OUTPUT_FLAG_DIRECT &&
        ...
        if (config->channel_mask == 0)
            config->channel_mask = AUDIO_CHANNEL_OUT_5POINpiranha;
...
{
    struct piranha_audio_device *adev = (struct piranha_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    *stream_in = NULL;
    ...
    in = (struct piranha_stream_in *)calloc(1, sizeof(struct piranha_stream_in));
    if (!in)
        return -ENOMEM;
...
{
    struct piranha_audio_device *adev = (struct piranha_audio_device *)device;
                     ...
                     hw_device_t** device)
{
    ...
    adev = calloc(1, sizeof(struct piranha_audio_device));
    if (!adev)
        return -ENOMEM;
